...
HEALTH_HEADERS="$(curl -sS -D - -o /dev/null "${BASE}/__servev2/health" || true)"
printf '%s' "${HEALTH_HEADERS}" | grep -iq '^x-frame-options: DENY' || fail "missing x-frame-options header"
printf '%s' "${HEALTH_HEADERS}" | grep -iq '^x-content-type-options: nosniff' || fail "missing x-content-type-options header"
printf '%s' "${HEALTH_HEADERS}" | grep -iq '^content-security-policy:' || fail "missing content-security-policy header"
...
printf '%s' "${INVALID_RAW}" | grep -Eiq '^HTTP/[0-9.]+ 400' || fail "invalid json did not return 400"
printf '%s' "${INVALID_RAW}" | grep -Eq '"code"[[:space:]]*:[[:space:]]*"invalid_json"' || fail "invalid_json error code missing"
...
rm -f "${BIG_FILE}"
printf '%s' "${LARGE_RAW}" | grep -Eiq '^HTTP/[0-9.]+ 413' || fail "large payload did not return 413"
printf '%s' "${LARGE_RAW}" | grep -Eq '"code"[[:space:]]*:[[:space:]]*"payload_too_large"' || fail "payload_too_large error code missing"
